...
export const useGitActions = () => ({
  doAddCommit: () => {
    ...
    useUIFeedback
      .getState()
      .flashAndLogCommit(result.command, before.HEAD, before, result.state);
  ...
  doCheckout: (target: string) => {
    ...
    if (mode === "module10") {
      ...
      useUIFeedback
        .getState()
        .flashAndLog(result.command, before, result.state);
  ...
  doReflogRecover: (hash: string) => {
    ...
    useUIFeedback
      .getState()
      .flashAndLog(`git reset --hard ${hash}`, result.before, result.after);
  ...
  resolveConflict: (resolution: "ours" | "theirs" | "both") => {
    ...
    const result = useGitEngine
      .getState()
      .applyMergeResolution(sourceBranch, targetBranch, resolution);
    if (!result) return;
    useUIFeedback
      .getState()
      .flashAndLog(result.command, result.before, result.after);
