...
export const Sidebar = ({ history, mode, moduleProgress, justUnlockedId, onEnter }: SidebarProps) => {
  ...
  return (
    <SidebarPanel className="w-56 bg-[var(--panel2)] p-3">
      <div className="mb-1 flex-shrink-0 font-hand text-lg font-bold text-[var(--ink)]">Giteractive</div>
      ...
      <div className="flex flex-shrink-0 flex-col gap-1 overflow-y-auto" style={{ maxHeight: 128 }}>
        {history.length === 0 ? (
          <div className="py-1 font-hand text-xs text-[var(--muted)]">{t('sidebar.noCommands')}</div>
        ) : (
          history.map((entry) => (
            <div
              key={entry.id}
              className="flex-shrink-0 border border-[var(--hair)] bg-[var(--panel)] p-1.5"
              style={{ borderRadius: cardRadius }}
            >
              <div className="font-mono text-xs text-[var(--ink)]">{entry.command}</div>
              <div className="mt-px font-mono text-xs text-[var(--muted)]">{timeAgo(entry.timestamp)}</div>
            </div>
          ))
        )}
      </div>
