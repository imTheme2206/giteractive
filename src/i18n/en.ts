const en = {
  ...
  intro: {
    module0: {
      ...
      scenario:
        "You have a folder of files and no version history. You're about to make your first commit — the moment a project stops being just a folder and becomes a tracked repository.",
      concept:
        '`git init` creates a hidden `.git` directory inside your folder. That directory is the entire repository — it stores every snapshot, branch, and piece of history. Nothing outside it is touched.',
      keyInsight:
        'Git is local-first. Your entire history lives on your machine. GitHub, GitLab, and Bitbucket are just mirrors — they host a copy of your `.git` directory online so others can collaborate.',
    },
    module1: {
      title: 'Module 1 — The Linear Timeline',
      scenario:
        'Every project starts somewhere. Git tracks your work as a chain of snapshots called commits. Each commit is permanent, named, and linked to the one before it — giving you a complete, reversible history of everything that ever changed.',
      concept:
        'When you run `git commit`, Git stores a snapshot of your staged changes and links it to the previous commit. HEAD and your branch label both point to the latest commit and advance with each new one.',
      keyInsight:
        'The staging area sits between your files and history. You choose exactly which changes go into each commit — giving you precise, meaningful snapshots instead of one giant "saved everything" blob.',
    },
    module2: {
      title: 'Module 2 — Parallel Universes',
      scenario:
        'You want to try a risky refactor without breaking the working version. In practice, you create a branch — an independent line of work you can develop, test, and throw away without ever touching main.',
      concept:
        'A branch is just a movable pointer to a commit — no files are copied. When you commit on a branch, the pointer advances to the new commit. Switching branches moves HEAD to a different pointer.',
      keyInsight:
        'Branches are nearly free. Creating one takes microseconds and copies nothing. The real cost is merging — so branch early, branch often, and keep branches short-lived.',
    },
    module3: {
      title: 'Module 3 — Cherry-pick',
      scenario:
        "You fixed a critical bug on a feature branch. Production needs that fix now — but the feature itself isn't ready. Cherry-pick lets you copy just that one commit onto main without bringing the rest of the branch along.",
      concept:
        "`git cherry-pick <hash>` takes a commit's diff — the exact changes it introduced — and re-applies them on top of your current branch tip. The new commit has the same content but a brand-new hash.",
      keyInsight:
        'The cherry-picked copy is completely independent of the original. Deleting, amending, or rebasing the original has no effect on the copy you made.',
    },
    module4: {
      title: 'Module 4 — Rebase',
      scenario:
        "Your feature branch fell behind while others merged work into main. Instead of cluttering history with a merge commit, you want your commits to sit cleanly on top — as if you'd started from the latest main all along.",
      concept:
        'Rebase finds the common ancestor of both branches, lifts your commits off their old base, and re-applies them one-by-one on top of the target. Each commit gets a new hash because its parent changed.',
      keyInsight:
        "Rebase rewrites history. Never rebase commits others have already pulled — you'll create divergent timelines that are painful to reconcile. Keep rebase local or use it only on your own branches.",
    },
    module5: {
      title: 'Module 5 — Merge',
      scenario:
        'Your feature is complete and reviewed. You want to bring it into main while keeping a full, honest record of when the feature work happened — exactly as it was written, without rewriting anything.',
      concept:
        '`git merge` creates a new merge commit with two parents — one from each branch. The original commit IDs are completely untouched; Git just adds a new node that ties both histories together.',
      keyInsight:
        'Unlike rebase, merge is non-destructive — both timelines are preserved exactly as they happened. The trade-off is a non-linear history. Teams that value auditability choose merge; teams that prefer clean logs choose rebase.',
    },
    module6: {
      title: 'Module 6 — Merge Conflicts',
      scenario:
        "Two developers edited the same line of the same file on different branches. When they try to merge, Git can't decide which version is correct — it stops and asks for human judgment.",
      concept:
        "When Git detects conflicting changes it can't auto-resolve, it pauses the merge and marks the collisions in the file. You resolve them by choosing which version (or a blend) to keep, then complete the merge.",
      keyInsight:
        "Conflicts are normal — not failures. They're Git asking you to make a decision it can't make alone. The more your team communicates about who owns which parts of the codebase, the fewer conflicts you'll encounter.",
    },
    module7: {
      title: 'Module 7 — git reset',
      scenario:
        "You made a few bad commits on main — experiments that shouldn't have been committed. No one else has pulled them yet. You want to erase them completely, as if they never existed.",
      concept:
        '`git reset --hard <hash>` moves the branch pointer back to a specific commit and discards everything after it. The commits disappear from history and the working tree is cleaned to match.',
      keyInsight:
        "`--hard` is permanent and unrecoverable through normal git commands. Only use it on commits you haven't shared. For public branches, use `git revert` instead — it adds a new commit that safely undoes the change.",
    },
    module8: {
      title: 'Module 8 — git stash',
      scenario:
        "You're halfway through a feature when an urgent bug report arrives. You need to switch branches, but your uncommitted changes can't come along — and you can't commit half-finished work.",
      concept:
        '`git stash` saves your uncommitted changes to a temporary stack and restores a clean working tree. You switch branches, fix the bug, come back, and pop the stash to resume exactly where you left off.',
      keyInsight:
        'The stash is a stack — you can push multiple times. `git stash pop` always restores the most recent entry first. Stashes persist even if you switch branches or restart your terminal.',
    },
    module9: {
      title: 'Module 9 — Interactive Squash',
      scenario:
        'You made three messy WIP commits on a feature branch. Before merging to main, you want to collapse them into one clean, descriptive commit — as if the work was done perfectly the first time.',
      concept:
        "`git rebase -i HEAD~3` opens an interactive editor where you can mark commits to squash. Git combines them into one, keeps the earliest commit's timestamp, and lets you rewrite the message.",
      keyInsight:
        'Squashing is rewriting history — never do it on commits others have pulled. Keep it local. A clean, atomic commit log is easier to review, bisect, and revert than a trail of "wip", "fix typo", "fix again".',
    },
    module10: {
      title: 'Module 10 — Detached HEAD',
      scenario:
        "You want to inspect an old commit without creating a branch. You run `git checkout <hash>` and suddenly git warns you: 'HEAD is now in detached state'. What does that mean?",
      concept:
        "HEAD normally points to a branch, which points to a commit. When you checkout a commit hash directly, HEAD points to the commit itself — 'detached' from any branch. Any commits you make here are orphaned when you leave.",
      keyInsight:
        'Detached HEAD is useful for exploration or testing. If you want to keep work done in detached state, create a branch immediately: `git checkout -b new-branch`. Otherwise, reattach to a named branch to get back to safety.',
    },
    module11: {
      title: 'Module 11 — Reflog Recovery',
      scenario:
        'You ran `git reset --hard` and your commits disappeared. Panic. But wait — git reset only moves the branch pointer. The actual commit objects are still in the repository for about 30 days.',
      concept:
        '`git reflog` shows every place HEAD has been — a time-stamped log of branch checkouts, commits, resets, and merges. Find the commit you lost, copy its hash, then `git reset --hard <hash>` to restore it.',
      keyInsight:
        "The reflog is your safety net. It's local-only (not pushed), so it won't help your teammate recover their lost commits — but it will save you. Git's garbage collector eventually prunes orphaned commits, so act fast.",
    },
  },
  tooltips: {
    head: {
      ...
      detached:
        'HEAD is detached — pointing directly at a commit hash, not a branch. Any commits made here will be orphaned when you switch away.',
  ...
  commandPanel: {
    ...
    commands: {
      commit: {
        title: 'git commit',
        description:
          'Snapshots your staged changes into a permanent new node in the graph. HEAD and your current branch advance to the new commit.',
      },
      checkout_b: {
      ...
      checkout: {
        title: 'git checkout',
        description:
          'Moves HEAD to a branch or commit hash. On a branch: HEAD follows it. On a hash: HEAD detaches — you enter exploration mode with no branch attached.',
      },
      cherry_pick: {
      ...
      rebase_i: {
        ...
        description:
          'Rewrites a sequence of commits interactively — squash collapses multiple messy commits into one clean commit before merging.',
        steps: [
      ...
      rebase: {
        title: 'git rebase',
        description:
          'Lifts your commits off their old base and re-applies them on top of the target branch. Each commit gets a new hash because its parent changed.',
        steps: [
      ...
      merge: {
        title: 'git merge',
        description:
          'Combines two branch histories with a new merge commit that has two parents. No existing commit is rewritten or moved.',
        steps: [
          'Git finds the common ancestor of both branches',
      ...
      reset_hard: {
        ...
        description:
          'Moves the branch pointer back to a specific commit, permanently discarding everything after it. Working tree is reset to match.',
        steps: [
      ...
      stash: {
        title: 'git stash',
        description:
          'Saves your uncommitted changes to a temporary stack and gives you a clean working tree, so you can safely switch branches.',
      },
      reflog: {
        title: 'git reflog',
        description:
          'Shows every place HEAD has ever been — commits, checkouts, resets, merges. Your safety net for recovering commits that seem lost after a hard reset.',
      },
    },
  },
  commandHistory: {
    ...
    modules: {
      module0: {
        commands: [
          {
            description:
              'Initializes a new repository in the current directory by creating a hidden .git folder. This folder is the entire repository.',
          },
          { description: 'Stages all changes in the working tree, queuing them for the next commit.' },
          {
            description:
              'Creates the first snapshot. With no parent commits, this is called a root commit — the anchor of your entire history.',
          },
        ],
      },
      module1: {
        commands: [
          ...
          {
            description:
              'Snapshots staged changes into a permanent new node in the history graph. HEAD and the current branch both advance to the new commit.',
          },
        ],
      },
      module2: {
        commands: [
          {
            description:
              'Creates a new branch pointer and switches to it immediately. No files are copied — a branch is just a movable label pointing at a commit.',
          },
      ...
      module3: {
        commands: [
          {
            description:
              "Copies one commit's exact diff onto the tip of your current branch. The copy gets a new hash — same changes, fresh identity.",
            steps: [
      ...
      module4: {
        commands: [
          {
            description:
              'Lifts your commits off their old base and re-applies them one-by-one on top of the target. History is rewritten — new hashes because parents changed.',
            steps: [
      ...
      module6: {
        commands: [
          {
            description:
              'When both branches edited the same lines, Git pauses and marks the collision. You choose which version to keep, then complete the merge.',
            steps: [
      ...
      module7: {
        commands: [
          {
            description:
              'Moves the branch pointer back to the target commit, permanently discarding everything after it. Working tree is reset to match.',
            steps: [
      ...
      module8: {
        commands: [
          {
            description:
              'Saves your uncommitted changes to a temporary stack and gives you a clean working tree — safe to switch branches without losing work.',
          },
      ...
      module9: {
        commands: [
          {
            description:
              'Opens an interactive rewrite session. Squash collapses N messy commits into one clean, descriptive commit before merging.',
            steps: [
      ...
      module10: {
        commands: [
          {
            description:
              'Checks out a commit hash directly, putting HEAD in detached state — pointing at the commit itself, not a branch. Commits made here are orphaned when you leave.',
          },
      ...
      module11: {
        commands: [
          {
            description:
              'Shows every position HEAD has ever occupied — commits, checkouts, resets, merges. Your safety net for finding commits that seem lost after a hard reset.',
          },
          {
            description:
              'Used with a reflog hash to restore a branch pointer to a previously reachable commit, bringing orphaned commits back into history.',
          },
        ],
      },
    },
