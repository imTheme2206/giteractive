const th = {
  ...
  intro: {
    module0: {
      ...
      scenario:
        'คุณมีโฟลเดอร์ไฟล์แต่ยังไม่มีประวัติเวอร์ชันใดเลย คุณกำลังจะทำ Commit แรก — จุดที่โปรเจกต์เปลี่ยนจากแค่โฟลเดอร์ธรรมดาให้กลายเป็น Repository ที่ถูก Track แล้ว',
      concept:
        '`git init` สร้างโฟลเดอร์ `.git` ที่ซ่อนอยู่ภายในโฟลเดอร์ของคุณ โฟลเดอร์นั้นคือ Repository ทั้งหมด — มันเก็บทุก Snapshot, Branch และประวัติการเปลี่ยนแปลง ไม่มีอะไรนอกจากนั้นถูกแตะต้อง',
      keyInsight:
        'Git ทำงานแบบ Local-first ประวัติทั้งหมดอยู่บนเครื่องของคุณ GitHub, GitLab และ Bitbucket เป็นแค่กระจกสะท้อน — พวกมันโฮสต์สำเนาของโฟลเดอร์ `.git` ออนไลน์เพื่อให้ทีมทำงานร่วมกันได้',
    },
    module1: {
  ...
  tooltips: {
    head: {
      ...
      detached:
        'HEAD หลุดออกจาก branch — กำลังชี้ตรงไปที่ commit hash ไม่ใช่ branch ถ้า commit ในสถานะนี้จะกลายเป็น orphan เมื่อเปลี่ยนไปที่อื่น',
  ...
  commandPanel: {
    ...
    commands: {
      ...
      checkout: {
        title: 'git checkout',
        description:
          'ย้าย HEAD ไปยัง branch หรือ commit hash ถ้าเป็น branch: HEAD จะติดตาม branch นั้น ถ้าเป็น hash: HEAD จะ detach — เข้าสู่โหมดสำรวจโดยไม่มี branch',
      },
      cherry_pick: {
      ...
      stash: {
        title: 'git stash',
        description:
          'บันทึกการเปลี่ยนแปลงที่ยังไม่ได้ commit ไปที่ stack ชั่วคราวและให้ working tree สะอาด เพื่อให้สลับ branch ได้อย่างปลอดภัย',
      },
      reflog: {
        title: 'git reflog',
        description:
          'แสดงทุกที่ที่ HEAD เคยอยู่ — commits, checkouts, resets, merges ตาข่ายนิรภัยสำหรับกู้คืน commit ที่ดูเหมือนหายไปหลัง hard reset',
      },
    },
  },
  commandHistory: {
    ...
    modules: {
      ...
      module2: {
        commands: [
          {
            description:
              'สร้าง branch pointer ใหม่และสลับไปทันที ไม่มีการคัดลอกไฟล์ — branch คือแค่ label ที่เคลื่อนที่ได้ซึ่งชี้ไปยัง commit',
          },
      ...
      module3: {
        commands: [
          {
            description:
              'คัดลอก diff ที่แน่ชัดของ commit หนึ่งไปยังปลาย branch ปัจจุบัน สำเนาได้ hash ใหม่ — การเปลี่ยนแปลงเหมือนกัน แต่เป็น identity ใหม่',
            steps: [
      ...
      module6: {
        commands: [
          {
            description:
              'เมื่อทั้งสอง branch แก้ไขบรรทัดเดียวกัน Git จะหยุดและทำเครื่องหมายการชน คุณเลือกว่าจะเก็บเวอร์ชันใด จากนั้น merge ต่อ',
            steps: [
      ...
      module7: {
        commands: [
          {
            description:
              'ย้าย branch pointer กลับไปยัง commit เป้าหมาย ทิ้งทุกอย่างหลังจากนั้นอย่างถาวร working tree จะถูก reset ให้ตรงกัน',
            steps: [
              'Branch pointer เลื่อนกลับไปยัง commit เป้าหมาย',
      ...
      module8: {
        commands: [
          {
            description:
              'บันทึกการเปลี่ยนแปลงที่ยังไม่ได้ commit ไปที่ stack ชั่วคราวและให้ working tree สะอาด — ปลอดภัยในการสลับ branch โดยไม่สูญเสียงาน',
          },
      ...
      module9: {
        commands: [
          {
            description:
              'เปิด session การเขียนใหม่แบบ interactive Squash รวม commit ที่ไม่เป็นระเบียบ N ตัวให้เป็น commit เดียวที่สะอาดก่อน merge',
            steps: [
      ...
      module10: {
        commands: [
          {
            description:
              'Checkout commit hash โดยตรง ทำให้ HEAD อยู่ใน detached state — ชี้ไปยัง commit เอง ไม่ใช่ branch commit ที่ทำที่นี่จะเป็น orphan เมื่อออกไป',
          },
      ...
      module11: {
        commands: [
          {
            description:
              'แสดงทุกตำแหน่งที่ HEAD เคยอยู่ — commits, checkouts, resets, merges ตาข่ายนิรภัยสำหรับหา commit ที่ดูเหมือนหายหลัง hard reset',
          },
