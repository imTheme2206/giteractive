...
type GitMutation<TArgs extends unknown[]> = (
  state: GitState,
  ...args: TArgs
...
type GitEngineStore = {
  ...
  doReflogRecover: (
    hash: string,
  ) => { before: GitState; after: GitState } | null;
  ...
  resetToState: (
    state: GitState,
    shadow?: Record<string, CommitNode>,
    reflog?: ReflogEntry[],
  ) => void;
};
...
export const useGitEngine = create<GitEngineStore>((set, get) => ({
  ...
  doCherryPick: (sourceId, targetBranch) =>
    runMutation(get, set, git.cherryPick, sourceId, targetBranch),
  ...
  doCreateBranch: (commitId) => {
    const branchName = git.getNextBranchName(
      Object.keys(get().gitState.branches),
    );
  ...
  doSquash: (branch, count, message) =>
    runMutation(get, set, git.squashCommits, branch, count, message),
  ...
  resetToState: (state, shadow = {}, reflog = []) =>
    set({ gitState: state, shadowCommits: shadow, reflog }),
}));
