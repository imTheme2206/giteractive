...
import type { GitState, ModuleId, ModuleProgress, ModuleStatus } from '../types'

const INITIAL_PROGRESS: ModuleProgress[] = Object.entries(MODULE_REGISTRY).map(([id, def]) => ({
  id: id as ModuleId,
...
export const useModuleFlow = create<ModuleFlowStore>((set, get) => ({
  ...
  setModuleStatus: (id, status) => get().setModuleProgress((prev) => prev.map((p) => (p.id === id ? { ...p, status } : p))),

  setModuleGuided: (v) => set({ moduleGuided: v }),

  setModuleAttempts: (updater) =>
    set((s) => ({
      moduleAttempts: typeof updater === 'function' ? updater(s.moduleAttempts) : updater,
    })),
  ...
  unlockAll: () => {
    get().setModuleProgress((prev) => prev.map((p) => (p.status === 'locked' ? { ...p, status: 'available' } : p)))
    set({ devMode: true })
  },
}))
